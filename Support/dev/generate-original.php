 ...
 *
 * Some languages may require memory_limit to 256MB in php.ini
 *
...
$outputDir = realpath(dirname(__DIR__, 2));
...
function getSectionName($id, $raw = false) {
	...
	while (isset($parentIds[$id]) && $parentId = $parentIds[$id]) {
		...
		if (substr($parentId, 0, 5) === 'book.') {
			$suffix = '.'.substr($parentId, 5);
		} elseif (substr($parentId, 0, 5) === 'refs.') {
	...
	exit("Could not determine section name for {$orig}");
}
...
function parseInfo($info) {
	...
	foreach ($info->params as $param) {
		...
		$param->optional = is_bool($param->optional) ? (bool) $param->optional : ($param->optional === 'true');
...
foreach ($dir as $fileinfo) {
	...
	} elseif (in_array($section, $excludeSections)) {
